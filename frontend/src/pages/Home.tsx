...
const Home: React.FC = () => {
  ...
  useEffect(()=>{
    (async ()=>{
      setLoading(true)
      const {data} = await axios.get(`http://localhost:3000/api/articles/${category}`)
      console.log(data.data)
      setFilteredArticles(data.data)
      setLoading(false)
    })()
  },[category,setCategory])
  // function setCate(txt){
  //   setCategory(txt)
  // }
      ...
      <div className="grid grid-cols-1 gap-8">
        {filteredArticles.length > 0 ? (
          <>
            {/* Featured Article */}
            <div className="mb-8">
              <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">Featured</h2>
          ...
          <div className="text-center py-12">
            <h3 className="text-xl font-medium text-gray-900 dark:text-white">No articles found</h3>
